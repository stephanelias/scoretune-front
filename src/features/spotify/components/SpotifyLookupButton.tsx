  ...
  return (
    <div className="shrink-0">
      <button
        ...
        className="inline-flex items-center justify-center size-10 rounded-lg bg-[#1DB954] text-white hover:bg-[#1ed760] focus:outline-none focus:ring-2 focus:ring-[#1DB954] focus:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none"
      >
        {isLoading ? (
          <span
            className="animate-spin inline-block size-4 border-2 border-current border-t-transparent rounded-full"
            role="status"
            aria-label="loading"
          />
        ) : (
      ...
      {error && <p className="mt-1 text-xs text-red-600 max-w-40">{error}</p>}
    </div>
  )
}
